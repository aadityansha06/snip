...
void writingfile(char path[PATH_MAX], char filename[100],
                 char prev_version[PATH_MAX]);
/* it will check for the file which user want
to commit let say main.c
it will find it in whole folder googin upward and then read data from it and
write it inside the prev-version i.e main-01.c */
...
/* detecting users OS and launching their IDE if avialble in side by side view
 to comapre code (i used chat gpt for this logic as i dotn know much about
 thses function) */
void open_diff_view(char *file1, char *file2) {
  ...
  if (system("where code >nul 2>&1") == 0) {
    snprintf(command, sizeof(command), "code --diff \"%s\" \"%s\"", file1,
             file2);
  ...
  if (system("which code > /dev/null 2>&1") == 0) {
    ...
    printf(
        "No supported editor found. Please open \"%s\" and \"%s\" manually.\n",
        file1, file2);
    return;
  }
...
int main(int argc, char *argv[]) {
  ...
  /*If you type snip with no further arguments in a terminal, again argc == 1,
   and interactive mode starts.*/
  if (argc == 1) {
    ...
    while (exit_request != 1) {
      ...
      /* printf("your command is %s", command);
       ...
       to that point. If none of the characters are found then the length of
      ...
      if (ret == 2 && strcmp(cmd, "snip") == 0) {
        ...
        } else if (strcmp(arrg, "commit") ==
                   0) // snip commit -hello.c -m"fixed-bugs" where m stands for
                      // mesggae
        {
          ...
          // exsit conatian file path till .snip/hello(folder for all hello.c
          // file)/
        ...
        } else if (strcmp(arrg, "log") == 0) // snip log -hello.c
        {
          ...
          if (path == NULL) {
            ...
            printf("snip file is not added pls use ->  snip add -filename or "
                   "snip --help for help\n");
          }

          readlog(exist); // exist contain path till folder except log file
          printlog();
        } else if (strcmp(arrg, "write") ==
                   0) // snip write -hello.c -1 where 1 stand for version 1
        {
        ...
        } else if (strcmp(arrg, "compare") ==
                   0) { // compare the vesrions with snip compare -hello.c v-3
                        // and v-4 ; one must commit before comapring
          char *path = checkvsc();
          char *exist = file_exist(command, path);
          if (path == NULL) {
            printf("snip is not initiated\n");
          ...
          printf("snip version 1.01 is currently running on your system \n "
                 "Check out : https://github.com/aadityansha06/snip for any "
                 "new realeases,feedback and contributation");
        } else if (strcmp(arrg, "--help") == 0) {
          help();
        } else if (strcmp(arrg, "--exit") == 0) {
          exit_request = 1;
        } else {
          printf("snip unable to recognise '%s' pls use snip init to get "
                 "started or use snip --help",
                 arrg);
        }
      } else {
        printf("\nunable to recognise '%s' pls use snip init to get started or "
               "use snip --help",
               command);
      }
    }
    ...
    if (ret == 1) {
      if (strcmp(arrg, "--version") == 0) {
        // Create full command for existing functions
        printf("snip version 1.01 is currently running on your system \n Check "
               "out : https://github.com/aadityansha06/snip for any new "
               "realeases,feedback and contributation");
      } else if (strcmp(arrg, "--help") == 0) {
        help();
      }
...
void initial(char command[100]) {
  struct stat
      s; // checking folder whether vsc(version-system control) exist or not
...
char *checkvsc() {
  // A static char variable, whether declared inside or outside a function, has
  // a lifetime that spans the entire duration of the program's execution. code
  char path[PATH_MAX];
  getcwd(path, sizeof(path));
  static char check_path[PATH_MAX];
  while (1) {
    snprintf(check_path, sizeof(check_path), "%s\\.snip",
             path); // combine path + %s// and put it in check path
    // it wil make a string  path\.snip ex-problems -c\snip\.snip and put into
    // chekc_path
...
void addfile(char command[100], char path[PATH_MAX]) {
  ...
  // so if we want to go like .snip/hello.c in direcotry ; we have to pass
  // .snip(path)//hello.c(token)

  // now we'll create a direcotry with thne name hello.c inside vsc which
  // contain all logs and hsitory of hello.c
  //  filevariable conatin .snip/hello.c

  struct stat
      s; // checking folder whether hello.c(version-system control) exist or not
...
char *file_exist(char command[100], char path[PATH_MAX]) {
  ...
  if (sscanf(command, "snip commit -%s", filename) == 1 ||
    ...
    char log_path[PATH_MAX];
    snprintf(file, sizeof(file), "%s\\%s", path, filename);
    struct stat s; // checking folder whether hello.c in .snip(version-system
                   // control) exist or not
...
void commit_msg(char command[100], char path[PATH_MAX]) {
  ...
  if (arggc == 1) {
    if (sscanf(cmd, "snip commit -%s", filename) != 1) {
      printf("Invalid command format. Use: snip commit -filename.ext "
             "[-m\"message\"]\n");
    }
  ...
  if (stat(check_log, &s) == 0 && S_ISREG(s.st_mode)) {
    ...
    snprintf(prev_version, sizeof(prev_version), "%s\\.version\\%d", path,
             commit_version);
    writingfile(path, filename, prev_version); // Pass filename, not full path
  } else {
    printf("log doesn't exist, please make sure your file and .snip are inisde "
           "same directory\n");
  }
}
...
void writingfile(char path[PATH_MAX], char filename[100],
  ...
  while (1) {
    ...
    // If we reach root, stop
    if (strlen(current_dir) <= 3 && current_dir[1] == ':' &&
        current_dir[2] == '\\') {
      break;
    }
  ...
  if (found != 1) {
    printf("\nNO such file named %s found!!\nPls check whether the file and "
           ".snip exist in same directory or not",
           filename);
  } else {
...
void overwrite(char command[100], char path[PATH_MAX]) {
  char ver_path[PATH_MAX]; // to find the path of actual code where the version
                           // is to be overwrite
  char current_dir[PATH_MAX];
  getcwd(current_dir, sizeof(current_dir));
  ...
  if (sscanf(command, "snip write -%s v-%d", filename, &ver) == 2) {
    snprintf(pre_version, sizeof(pre_version), "%s\\.version\\%d", path,
             ver); // Fixed path separators
    struct stat s;
    ...
    while (1) {
      ...
      if (stat(ver_path, &s) == 0 && S_ISREG(s.st_mode)) {
        ...
        if (current == NULL || pre_ver == NULL) {
          printf("Error during reading and writing file or version %d doesn't "
                 "exist\n",
                 ver);
          exit(1);
        }
    ...
    printf("Invalid command format. Use: snip write -filename.ext version-num "
           "(e.g v-3 for version 3) \n");
    return;
  }

  if (found == 0) {
    printf("File not found: %s\n please make sure your filename is correct or "
           "your file and .snip exist in same folder\n",
           filename);
  }
}
...
void compare(char command[100], char filepath[PATH_MAX]) {
  ...
  if (sscanf(command, "snip compare -%s v-%d and v-%d", filename, &ver1,
             &ver2) == 3) {
    char current_dir[PATH_MAX];
    ...
    if (stat(ver_path1, &s1) == 0 && S_ISREG(s1.st_mode) &&
      ...
      snprintf(temp_path1, sizeof(temp_path1), "%s\\%d-%s", current_dir, ver1,
               filename);
      snprintf(temp_path2, sizeof(temp_path2), "%s\\%d-%s", current_dir, ver2,
               filename);
      ...
      while (del != 1) {
        ...
        if (del == 1) {
          ...
          if (remove(temp_path2) == 0)
            printf(
                "Comparision closed sucessfully Now you can remove the file\n");
          else
            printf("Error deleting %s\n", temp_path2);
        } else {
    ...
    printf("Unable to find one of the versions. Check with 'snip log "
           "-filename.ext'\n");
    exit(1);
  }
}
void printlog() {
  ...
  if (start == NULL) {
    ...
    printf("\t+-----+----------------------------------------------------------"
           "-------------------+\n");
    printf("\nUse snip write \"-filename -version\"..  to overwrite previous "
           "version\n");
  }
}
...
void help() {
  ...
  printf("\n\t| %-40s | %-70s\n", "Command", "\t\tDiscription");
  printf("\n\t| %-40s | %-70s\n", "snip ",
         "\t\tTo start with command line argument directly  for init,commit "
         "compare,view-logs and over-write");
  printf("\n\t| %-40s | %-70s\n", "snip init",
         "\t\tTo initialize snip for that particular folder "
         "\n\t\t\t\t\t\t\t\t!!!It will only work with the file inside that "
         "folder which contain '.snip' and the subfolder of that folder "
         "containing '.snip' ");
  printf("\n\t| %-40s | %-70s\n", "snip add -filename.ext",
         "\t\tTo add the file for the first time , so that any chnages made to "
         "them later will be stored here");
  printf("\n\t| %-40s | %-70s\n", "snip commit -filename.ext -m\"commit-msg\" ",
         "\t\tTo save the changes made to that file along with the msg that "
         "what chnages has made ");
  printf("\n\t| %-40s | %-70s\n", "snip log -filename.ext",
         "\t\tTo see the all previous commit with timestamp and it's version "
         "as 1,2,... !!!'Remember the version shown at top will be the recent "
         "version'");
  printf("\n\t| %-40s | %-70s\n",
         "snip write -filename.ext v-2 (e.g. v-2 for version-2)",
         "\t\tTo overwrite the previous version to your current file");
  printf("\n\t| %-40s | %-70s\n", "snip compare -filename.ext v-2 and v-3 ",
         "\t\tTo compare the desired version of  your current file,!commit "
         "before comparing!");
  printf("\n\t| %-40s | %-70s\n", "snip --version",
         "\t\tTo check the your  version or to download the latest version, "
         "also to check out the repo link for feedback and contributation");
